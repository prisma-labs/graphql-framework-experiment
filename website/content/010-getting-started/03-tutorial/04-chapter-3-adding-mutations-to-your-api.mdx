...
+     },
+   ],
+   contextType: "ctx.Context",               // 3
+ },
})
```

1. Behind the scenes Nexus will parse the `context.ts` file to extract the available types.
1. These types become available under the umbrella of our `ctx` alias.
1. We can then use that alias to refer to any type defined in this file. Here, our context type is named `Context`, so we refer to it as `ctx.Context`.
